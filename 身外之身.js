...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            text += "#L1##k" + aaa + "#r关闭身外之身#k    #L2#" + aaa + "#b购买身外之身#v3994759##n\r\n\r\n"
			

            cm.sendSimple(text);
        } else if (status == 1) {
            if (selection == 0) {//活动传送
                if (cm.getPlayer().getBossLog("身外之身",0) >= 1 ) {
				    ...
				    //cm.全服漂浮喇叭("恭喜大佬["+cm.getName()+"]：成功开启双重分身,战斗力额外提升翻倍！！", 5121004);
					
					cm.getPlayer().cloneLook();
					cm.getPlayer().setBossLog("身外之身",0);
					cm.sendOk("成功开启身外之身！");
					cm.dispose();
				}
                ...
                if (cm.getPlayer().getBossLog("身外2",0) >= 1 ) {
                ...
                if (cm.getPlayer().getBossLog("身外之身",1) < 1 ) {
					cm.sendOk("你还未开启身外之身，无需关闭。");
					cm.dispose();
				} else {
					cm.getPlayer().setBossLog("身外之身",1,-1);
                ...
                if (cm.getPlayer().getBossLog("身外2",1) < 1 ) {
					cm.sendOk("你还未开启身外之身，无需关闭。");
					cm.dispose();
				} else {
					cm.getPlayer().setBossLog("身外2",1,-1);
					cm.getPlayer().disposeClones();
					cm.sendOk("已关闭身外之身,如需要请重新开启~");
					cm.dispose();
                 }
