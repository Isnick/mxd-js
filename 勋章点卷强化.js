...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			...
			cm.sendOk("#b第一格子无东西或者第一格物品不是可强化勋章#k");	
			cm.dispose();
			} 	
		} else if (status == 2) {
			...
			if(cm.getInventory(1).getItem(1)== null ){
		            cm.sendOk("请把要升星的勋章放在第一格才能进行.");
				    cm.dispose();
			}
             else if (imtenuber<0 ) {
             cm.sendOk("第一格装备#v" + item.getItemId() + "#,不是可强化勋章!");
			 cm.dispose();    
			 return;
            }
				...
				cm.sendOk("亲，恭喜您已经强化到最高等级★★★★★+10，我已经没有能力再为它提升了哦！");
				cm.dispose();
			}else if(xx == "★★★★★"){
			...
			cm.sendOk("升星成功！");
			cm.全服黄色喇叭(  " 【勋章强化】" + " : " + "["+cm.getPlayer().getName()+"]勋章成功强化至"+xxnew+"！大家祝贺他(她)吧！！")
			cm.dispose();
				}
			}
	}}
