	...
	if (mode == -1) {
		...
		if (status == 0) {
			...
			if(cm.getInventory(1).getItem(1)== null ){
             ...
             cm.sendOk("第一格装备是#v" + item.getItemId() + "#,只有蓝调戒指才可以来升星!");
			 cm.dispose();    
			 return;
            }
				...
				sj = Math.floor(Math.random()*100);//随机50%
				shibai = 0;
				xxnew = "☆";
				快乐数量 = 0;//2
				快乐戒指 = 1112793;
				...
				sj = Math.floor(Math.random()*35);//35%
				shibai = 0;
				xxnew = "★☆☆☆☆";
				xxdj = 5;
				快乐数量 = 2;//2
				...
				sj = Math.floor(Math.random()*25);//25
				shibai = 0;
				xxnew = "★★★★★";
				xxdj = 9;
				快乐数量 = 3;//3
			...
			if(!cm.haveItem(快乐戒指,快乐数量)){
				cm.sendOk("#v"+快乐戒指+"#物品数量不足"+快乐数量+"个！");
				cm.dispose();
				return;
				}
			...
			cm.sendOk("升星成功！");
			cm.全服黄色喇叭(  " 【饰品升星】" + " : " + "["+cm.getPlayer().getName()+"]蓝调戒指成功提升星之力至"+xxnew+"！大家祝贺他(她)吧！！")
			cm.dispose();
				}
			}
	}}
