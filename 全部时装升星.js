...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			cm.sendNext("\t     "+彩虹+"  #e#d 时 装 升 星#k#n  #r  "+彩虹+"#b#k#n\r\r\n  "+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+"\r\n\r\n   #b欢迎来到#e" + cm.getChannelServer().getServerName() + "时装升星中心\r\n#k#n1.在我这里可以使用点卷进行升星!\r\n2.需要升星的时装请放在装备栏第一个格!#k\r\n");
				} else if (status == 1) {
			...
			{   
				...
				if (Packages.server.MapleItemInformationProvider.getInstance().isCash(cm.getInventory(1).getItem(1).getItemId()) != true) {//这个判定道具不是时装
                 cm.sendOk("不是时装，无法升星。");
                 cm.dispose();
                 return;
                }
			...
			cm.sendOk("升星成功！");
			cm.全服黄色喇叭(  " 【时装升星】" + " : " + "["+cm.getPlayer().getName()+"]时装成功提升星之力至"+xxnew+"！大家祝贺他(她)吧！！")
			cm.dispose();
				}
			}
	}}
