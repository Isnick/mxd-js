...
function action(mode, type, selection) {
    ...
    if (mode == 1) {
        ...
        if(status==0)
		{
				...
				text += "#d哈喽！有没有羡慕过别人的技能呢？如果有，给我888元宝就可以偷偷的教你学习到哦.同个技能第二,三次学习半价呢!#k\r\n"
				
				text+="#r点击学习好技能，即可在键盘设定F12键拿取技能使用！#k\r\n"
				text+="#r学习影分身,可以获得双倍伤害.#k\r\n"
				text+="#r"+点卷图标+"当前元宝："+cm.getmoneyb()+"  #k\r\n"
				for(i=0;i<newskillId.length;i++)//
				{
				if(newskillId[i]!=0){	
		        text+=" #L"+newskillId[i]+"##s"+newskillId[i]+"#  [#r学习#k] 当前等级:"+cm.getPlayer().getSkillLevel(newskillId[i])+" - 最高等级:"+cm.getPlayer().getMasterLevel(newskillId[i])+"#l \r\n"
				}
				}
				cm.sendSimple(text);
