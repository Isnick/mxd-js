...
function action(mode, type, selection) { 
    if (mode == -1) { 
        ...
        if (status == 0) { 
	    abb = 1;
	    cm.sendGetText("您好,欢迎使用金币增加HP.每"+cost+"金币增加1点最大HP值。\r\n目前拥有"+金币图标+"：#r"+cm.getMeso()+" #k\r\n#r注：请输入想增加的#b血量值#r..最大HP不能超过3万,否则出错该不负责");
        } else if (status == 1) { 
	if(cm.getText() < 1){
	    ...
	    cm.sendYesNo("您好,欢迎使用金币增加血量.\r\n增加#r" + cm.getText() + "#k血量将会使用掉您#r" + cm.getText() * cost + "#k金币\r\n请确认后使用。"); 
	    } 
        } else if (status == 2) { 
		var getmaxhp = cm.getPlayer().getStat().getMaxHp();
	if (cm.getMeso() >= cm.getText()*cost) { 
		   cm.gainMeso(-cm.getText() * cost);
		   cm.getPlayer().getStat().setMaxHp(getmaxhp+cm.getText() * 1);
           ...
           cm.sendOk("成功增加了"+cm.getText()+"您增加的HP.换线或小退一下即可看到。");
           cm.dispose();
        } else {
