...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		...
		selStr += "          天空副本-通关["+cm.getBossLog("天空副本")+"/5]次#n\r\n"    
		selStr += "          毒物副本-通关["+cm.getBossLog("毒物副本")+"/5]次#n\r\n" 
        selStr += "          海盗副本-通关["+cm.getBossLog("海盗副本")+"/3]次#n\r\n" 		
        ...
        case 0:
            if (cm.getBossLog("废弃副本") < 5){
				...
				cm.dispose();
			}else if (cm.getBossLog("天空副本") < 5){
				cm.sendOk("天空副本-通关["+cm.getBossLog("天空副本")+"/5]次");
				cm.dispose();
			}
			else if (cm.getBossLog("毒物副本") < 5){
				cm.sendOk("毒物副本-通关["+cm.getBossLog("毒物副本")+"/5]次");
				cm.dispose();
			}else if (cm.getBossLog("海盗副本") < 3){
				cm.sendOk("海盗副本-通关["+cm.getBossLog("海盗副本")+"/3]次");
				cm.dispose();
