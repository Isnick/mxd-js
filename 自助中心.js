...
function action(mode, type, selection) {
    ...
    if (mode == -1) {
        ...
        if (status == 0) {
			...
			//text += "#r赞助比例 1:"+比例+" 1元宝="+比例+"点卷 #k\r\n\r\n"	
		//	text += "\t\t"+点卷图标+"累计赞助："+累计充值+"\r\n"
			text += "\t\t"+点卷图标+"当前元宝："+余额+" \r\n"
			text += "\t\t"+点卷图标+"当前点卷："+点卷+" \r\n"
			
		//	if(充值点()>0){	
		//		text += "#L999##e#d - <<<赞助提现元宝>>> - "+点卷图标+"当前余额："+充值点()+"#l#k\r\n"
		//	}
		//	if (余额 > 0) {
		//		text += "#L2##e#b - <<<元宝兑换点卷>>> - "+点卷图标+"当前余额："+元宝+"#l#k\r\n\r\n"
		...
		//	} 
			...
			}else if (selection == 0) { //月卡会员
                   cm.dispose();
                    if (cm.getmoneyb() >= 88 && !cm.haveItem(5010041, 1)) {
                     cm.setmoneyb(-88);
                       cm.gainGachaponItemTime(5010041, 1, "月卡会员", 30);
                        cm.sendOk("#e#b[提示]：\r\n兑换成功月卡~\r\n");
                        return;
                    } else {
                        cm.dispose();
